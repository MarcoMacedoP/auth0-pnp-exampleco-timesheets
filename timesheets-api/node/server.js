...
const checkJwt = jwt({
  // Dynamically provide a signing key based on the
  //kid in the header and the singing keys provided by the JWKS endpoint.
...
app.post("/timesheets/upload", checkJwt, jwtAuthz(["batch:upload"]), function(
  req,
  res
) {
...
app.post("/timesheets", checkJwt, jwtAuthz(["create:timesheets"]), function(
  req,
  res
) {
  var timesheet = req.body;
...
// create timesheets API endpoint
app.get("/timesheets", checkJwt, jwtAuthz(["read:timesheets"]), function(
  req,
  res
) {
  // Get timesheet entries for this user
  var userEntries = timesheets.filter(
...
app.put("/approvals/:id", checkJwt, jwtAuthz(["approve:timesheets"]), function(
  req,
  res
) {
...
app.get("/approvals", checkJwt, jwtAuthz(["approve:timesheets"]), function(
  req,
  res
) {
